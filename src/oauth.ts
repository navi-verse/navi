...
export const DEFAULT_MODELS: Record<string, { provider: string; modelId: string }> = {
	...
	"github-copilot": { provider: "github-copilot", modelId: "claude-sonnet-4.6" },
	"google-gemini-cli": { provider: "google-gemini-cli", modelId: "gemini-3.1-pro-preview" },
	"google-antigravity": { provider: "google-antigravity", modelId: "gemini-3.1-pro-high" },
	"openai-codex": { provider: "openai-codex", modelId: "gpt-5.4" },
};

// Pending input handlers per WhatsApp JID — allows the OAuth callbacks
// to "await" a reply from a specific user.
const pendingInputs = new Map<string, { resolve: (value: string) => void; reject: (reason: Error) => void }>();
...
export function listProviders(authStorage: AuthStorage): string {
	...
	return ["Available login providers:", "", ...lines, "", "Reply with the number or provider name, e.g. /login 1"].join(
		"\n",
	);
}
 ...
 */
export function resolveProvider(authStorage: AuthStorage, input: string): string | null {
	const providers = authStorage.getOAuthProviders();
	...
	const partial = providers.find((p) => p.id.includes(lower) || p.name.toLowerCase().includes(lower));
 ...
 */
export async function performLogin(authStorage: AuthStorage, providerId: string, ctx: LoginContext): Promise<void> {
